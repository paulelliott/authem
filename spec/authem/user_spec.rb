...
describe Authem::User do
  ...
  describe 'validations' do
    ...
    context 'when email is not present' do
      let(:invalid_user) { PrimaryStrategyUser.create(email: nil) }
      its(:errors) { should include(:email) }
    end

    context 'when email has already been taken' do
      let(:invalid_user) { PrimaryStrategyUser.create(email: 'someone@example.com') }
      its(:errors) { should include(:email) }
    end

    context 'when email has an invalid format' do
      let(:invalid_user) { PrimaryStrategyUser.create(email: 'someone') }
      its(:errors) { should include(:email) }
    end
  end
  ...
  describe '.find_by_email' do
    subject { PrimaryStrategyUser.find_by_email(email) }
